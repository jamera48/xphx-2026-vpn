...
تيليجرام[FFFFFF]: [00FFFF]@TranBaoDev [FFFFFF]& [%s]تيك توك[FFFFFF]: [00FFFF]@abcxyz_bgt\n[000000]""" % (
    ...
    def gringay(self, client, recv):
        ...
        # 🔥 منع التكرار
        # ============================================
        msg_key = f"{cid}:{message}"
        current_time = time.time()
        
        # التحقق من تكرار الرسالة
        if msg_key in socks5._processed_messages:
            # إذا تكررت خلال 5 ثواني، تجاهلها
            if current_time - socks5._last_message_time.get(msg_key, 0) < 5:
                return True
        else:
            socks5._processed_messages.add(msg_key)
            # منع تراكم الذاكرة
            if len(socks5._processed_messages) > 1000:
                socks5._processed_messages.clear()
        
        socks5._last_message_time[msg_key] = current_time
