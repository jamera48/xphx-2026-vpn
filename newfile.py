...
# ===== إعدادات المستخدم =====
USERNAME = "bot"  # اسم المستخدم (يمكنك تغييره)
PASSWORD = "bot"  # كلمة المرور (يمكنك تغييرها)
SERVER_PORT = 3000  # البورت
# ============================
...
class SOCKS5_SERVER:
    def __init__(self):
        ...
        self.socks1200 = None
        self.auth_attempts = {}  # تخزين محاولات المصادقة
    ...
    def client_connect(self, cnn, sport, saddr):
        ...
        if 0x00 in methods:
            ...
            if username == USERNAME and password == PASSWORD:
                cnn.sendall(bytes([0x01, 0x00]))  # نجاح المصادقة
            else:
                cnn.sendall(bytes([0x01, 0x01]))  # فشل المصادقة
                cnn.close()
                return False
        else:
    ...
    def connect(self, host="0.0.0.0", port=SERVER_PORT):
        try:
            print(f"[+] تشغيل الخادم على البورت {port}")
...
Telegram[FFFFFF]: [00FFFF]@TranBaoDev [FFFFFF]& [%s]TikTok[FFFFFF]: [00FFFF]@abcxyz_bgt\n[000000]""" % (
            grcolor(),
            grcolor(),
            grcolor(),
        )
...
if __name__ == "__main__":
    SS = SOCKS5_SERVER()
    # استخدام البورت المحدد في المتغير SERVER_PORT
    threading.Thread(target=SS.connect, args=("0.0.0.0", SERVER_PORT)).start()
    print(f"[+] الخادم قيد التشغيل على البورت {SERVER_PORT}")
    print("[+] في انتظار الاتصالات...")
    
    # الحفاظ على تشغيل الخادم
    while True:
        time.sleep(1)